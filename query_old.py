...
class Query():
    ...
    def __init__(self, query):
        ...
        self.descr_and_scores = self.descr_vector_store.similarity_search_by_vector_with_score(self.embedding, k = 10)
    ...
    def grab_uuids_and_abbrevs(self, returns):
        """
            Given a set of sql returns, returns a list of tuples containing the uuid and dept_abbrv for each class in returns"""

        if '*' in self.sql.splitlines()[0]: 
    ...
    def find_where_clause(self):
        """
            Finds the where clause in the sql statement and returns it as a string. The original formatting is kept i.e. \n
        """
    ...
    def widen_search_sql(self, course_ids):
        """
        ...
        sql_final = f"SELECT uuid, department_abbrv, course_id FROM all_classes WHERE UUID IN ({formatted_ids})"
    ...
    def widen_search(self, iteration = 0, max_iterations = 5, last_where_idx = None, first_where_idx = None, prev_return_length = None, prev_result_uuids = None):
        ...
        if prev_result_uuids is None:
            prev_results_raw = self.run_sql()
            prev_result_uuids = self.grab_course_uuids(prev_results_raw)
       ...
       #didnt expand resutls
            if returns and len(returns) > prev_return_length: 
                new_returns = []
                return_uuids = self.grab_course_uuids(returns)
                for row in return_uuids:
                    for prev_row_uuid in prev_result_uuids:
                        if row == prev_row_uuid:
        ...
        # If no results and we can still widen and try again (limit to 5 attempts)
            return self.widen_search(iteration + 1, prev_return_length = len(returns), prev_result_uuids = prev_result_uuids)
